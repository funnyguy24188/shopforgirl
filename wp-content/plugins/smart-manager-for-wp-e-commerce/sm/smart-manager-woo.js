...
//Variations
		...
		var productManageStockCombo = new Ext.form.ComboBox({
			typeAhead: true,
			id: 'productManageStockCombo',
			triggerAction: 'all',
			lazyRender:true,
			editable: false,
			mode: 'local',
...
//custom columns for Products Dashboard
		...
		products_columns = [editorGridSelectionModel,
			...
			{
				header: SM.productsCols._manage_stock.name,
				id: 'manage_stock',
				hidden: false,
				sortable: true,
				dataIndex: SM.productsCols._manage_stock.colName,
				tooltip: getText('Manage Stock'),
				width: 100,
				renderer: Ext.util.Format.comboRenderer(productManageStockCombo)
			},
			{
...
// Code to create render fields array for products dashboard
		...
		products_render_fields = [
			...
			{name: SM.productsCols._manage_stock.colName, 	  type: 'string'},
		...
		var productsStore = new Ext.data.Store({
			...
			listeners: {
				...
				load: function (store,records,obj) {
					...
					productsColumnModel.getColumnById('manage_stock').editor = productManageStockCombo;
					productsColumnModel.getColumnById('visibility').editor = visibilityCombo;
					productsColumnModel.getColumnById('taxStatus').editor = taxStatusCombo;
					productsColumnModel.getColumnById('product_type').editor = productTypeCombo;
				}
