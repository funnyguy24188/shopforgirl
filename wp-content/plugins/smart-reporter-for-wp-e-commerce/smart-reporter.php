...
Copyright (c) 2011, 2012, 2013, 2014, 2015, 2016 Store Apps All rights reserved.
...
function sr_overwrite_site_transient( $plugin_info ) {
	...
	if (file_exists ( (dirname ( __FILE__ )) . '/pro/sr.js' ) && empty($sr_license_key)) {
...
function sr_upgrade() {
	if (file_exists ( (dirname ( __FILE__ )) . '/pro/sr.js' )) {
		...
		$documentation_link = '';
		$GLOBALS['smart_reporter_upgrade'] = new Store_Apps_Upgrade( __FILE__, $sku, $prefix, $plugin_name, SR_TEXT_DOMAIN, $documentation_link );
	}
}

function sr_add_support_ticket_content() {

	if ( empty($_GET['page']) || (!empty($_GET['page']) && $_GET['page'] != 'wc-reports' && $_GET['tab'] != 'smart_reporter' && $_GET['tab'] != 'smart_reporter_old'
	...
	if ( class_exists( 'Store_Apps_Upgrade' ) ) {
		...
		Store_Apps_Upgrade::support_ticket_content( SR_PREFIX, SR_SKU, $sr_plugin_data, $sr_license_key, SR_TEXT_DOMAIN );
	}
}
...
function is_pro_updated() {
	...
	function sr_schedule_daily_summary_mails() {
        ...
        $active_sitewide_plugins_option = get_option( 'active_sitewide_plugins' );

        $active_plugins = (!empty($active_plugins_option)) ? $active_plugins_option : array();
        $active_sitewide_plugins = (!empty($active_sitewide_plugins_option)) ? $active_sitewide_plugins_option : array();

        $sr_woo_active = (!empty($active_plugins)) ? in_array( 'woocommerce/woocommerce.php', $active_plugins) : FALSE;
        $sr_wpsc_active = (!empty($active_plugins)) ? in_array( 'wp-e-commerce/wp-shopping-cart.php', $active_plugins) : FALSE;
        $sr_woo_multi_active = (!empty($active_sitewide_plugins)) ? in_array( 'woocommerce/woocommerce.php', $active_sitewide_plugins) : FALSE;
        $sr_wpsc_multi_active = (!empty($active_sitewide_plugins)) ? in_array( 'wp-e-commerce/wp-shopping-cart.php', $active_sitewide_plugins) : FALSE;

		if ( ( !empty($active_plugins) && $sr_woo_active === TRUE ) || ( is_multisite() && $sr_woo_multi_active === TRUE ) || 
        	( !empty($active_plugins) && $sr_wpsc_active === TRUE ) || ( is_multisite() &&  $sr_wpsc_multi_active === TRUE) ) {
...
if ( is_admin () || ( is_multisite() && is_network_admin() ) ) {
 ...
 //    }
		...
		if ( defined('SRPRO') && SRPRO === true ) {
			...
			if ( is_admin() ) {
				if(isset($_GET['sr_dismiss_admin_notice']) && $_GET['sr_dismiss_admin_notice'] == '1'){
					...
					add_action( 'admin_notices', 'sr_add_promo_notices');
				}
		    }
		}
	...
	// Function to handle SR IN App Promo
	function sr_add_promo_notices() {
		...
		$sr_lite_activation_date = get_option('sr_lite_activation_date');

		if ( $sr_lite_activation_date === false ) {
			
			$sr_lite_activation_date = $current_wp_date;
			add_option('sr_lite_activation_date',$sr_lite_activation_date);
		}

		$date_diff = date_diff( date_create($sr_lite_activation_date),date_create($current_wp_date) );

		$sr_resp_msg = '<b>'. __('Congratulations!!!', SR_TEXT_DOMAIN) .'</b> ' . __('Kindly check your mail to avail the discount', SR_TEXT_DOMAIN) ;
		$sr_promo_cond = __('*Only For Today*', SR_TEXT_DOMAIN);
		$sr_promo_hide_msg = __('No, I don\'t like offers...', SR_TEXT_DOMAIN);

		if ( $date_diff->days == 0 ) {
			$sr_promo_msg = '<b>'. __('Big Savings!!!', SR_TEXT_DOMAIN) .' </b> <span style="color:#E34F4C;font-weight:bold;">' . __('20% OFF ', SR_TEXT_DOMAIN) . ' </span>' . __('on Smart Reporter Pro!', SR_TEXT_DOMAIN);
			$sr_klawoo_list_id = 'u41YRkCRFJZSK892FeR4I3zg';
		} else if ( $date_diff->days == 1 ) {
			$sr_promo_msg = '<b>'. __('Missed yesterday?', SR_TEXT_DOMAIN) .' </b> <span style="color:#E34F4C;font-weight:bold;">' .  __('15% OFF ', SR_TEXT_DOMAIN) . ' </span>' . __('on Smart Reporter Pro', SR_TEXT_DOMAIN);
			$sr_klawoo_list_id = 'IfRD3x02PPvBN2Y4ZKZYjA';
		} else if ( $date_diff->days == 2 ) {
			$sr_promo_msg = '<b>'. __('Last chance!!!', SR_TEXT_DOMAIN) .' </b> <span style="color:#E34F4C;font-weight:bold;">' . __('10% OFF ', SR_TEXT_DOMAIN) . ' </span>' . __('on Smart Reporter Pro!', SR_TEXT_DOMAIN);
			$sr_klawoo_list_id = '892l763hAC763B9z4892kVGP8KgpQA';
		} else if ( $date_diff->days > 2 ) {
			$sr_promo_msg = '<b>'. __('Sign up to get updates, insights & tips...', SR_TEXT_DOMAIN);
			$sr_klawoo_list_id = 'uYTAjoLlzo2arPsKE7dtuA';
			$sr_resp_msg = '<b>'. __('Thank you for Subscribing!!!', SR_TEXT_DOMAIN) .'</b>';
			$sr_promo_cond = '';
			$sr_promo_hide_msg = __('No, I don\'t need help...', SR_TEXT_DOMAIN);
		}
		...
		if ( !empty($sr_promo_msg) ) {
			...
			echo '<div id="sr_promo_msg" class="updated fade" style="display:block !important;font-size:1.1em;font-weight:100;"> 
					<table style="width:100%;"> 
						<tbody> 
							<tr>
								...
								<td id="sr_promo_msg_content" style="padding:0.5em;">
									<div style="padding-top:0.3em;float:left;">'. $sr_promo_msg .'</div>
									<form name="sr_klawoo_subscribe" action="#" method="POST" accept-charset="utf-8" style="float:left;padding-left:0.5em;">
			                            <input class="regular-text ltr" type="text" name="email" id="email" placeholder="Email" style="width:18em;height:1.75em;"/>
			                            <input type="hidden" name="list" value="'. $sr_klawoo_list_id .'"/>
			                            <span id="resp_message" style="display:none;">'. $sr_resp_msg .'</span>
			                            <input type="submit" name="submit" id="submit" class="button button-primary" value="Subscribe" style="height:1.75em;line-height:0;">
			                        </form>
			                        <div id="sr_promo_valid_msg">'. $sr_promo_cond .'</div>
			                        <div style="padding-top: 0.5em;font-size:0.8em;width:100%;float:left;">
			                        	<div style="float:left;"><a href="http://www.storeapps.org/product/smart-reporter" target=_storeapps> '. __( 'Learn more about Pro version', SR_TEXT_DOMAIN ) . '</a> ' . __( 'or take a', SR_TEXT_DOMAIN ) . ' <a href="http://demo.storeapps.org/?demo=sr-woo" target=_livedemo> ' . __( 'Live Demo', SR_TEXT_DOMAIN ) . ' </a>	</div>
										<div style="float:right;font-size:0.9em;"><a href="'.$current_url.'&sr_dismiss_admin_notice=1">'. $sr_promo_hide_msg .'</a></div>
									</div>
								</td> 
							</tr>
						</tbody> 
				...
				<script type="text/javascript">
		            jQuery(function () {
		                jQuery("form[name=sr_klawoo_subscribe]").submit(function (e) {
		                    e.preventDefault();
		                    
		                    params = jQuery("form[name=sr_klawoo_subscribe]").serializeArray();
		                    params.push( {name: "action", value: "sr_klawoo_subscribe" });
		                    ...
		                    jQuery.ajax({
		                        ...
		                        success: function(response) {
		                            ...
		                            if (resp.find("h2").text() == "You\'re subscribed!") {
		                                jQuery("td[id=sr_promo_msg_content]").html(jQuery("form[name=sr_klawoo_subscribe]").find("#resp_message").html());
	...
	function load_sr_woo_order_items( $wpdb ) {
		...
		$order_count = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}posts
