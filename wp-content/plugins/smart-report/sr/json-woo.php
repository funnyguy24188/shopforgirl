				...
				foreach ( $results as $row ) {
					...
					foreach ( $data as &$arr ) {
						...
						foreach ($arr as &$value) {
							...
							if ( !empty($row['id']) && $row['id'] == $value['id'] ) {
								$value['title'] = $row['title'];
							} else if ( !empty($index) && $index == $value['id'] ) {
								$value['title'] = $row['title'];
								unset($v_ids[$index]);
							}
	...
	function sr_query_sales($cumm_dates,$date_series,$post) {
		...
		if( !empty($post['cmd']) && ($post['cmd'] == 'cumm_cust_prod' || $post['cmd'] == 'sr_summary') ) {
			...
			if ( count($results) > 0 ) {
				...
				foreach ( $results as $row ) {
					...
					$returns['kpi']['top_prod']['sales'][] = array( 'id' => $id,
																	'title' => '-',
																	'sales' => $row['sales'], 
																	'qty' => $row['qty'] );
			...
			if ( count($results) > 0 ) {
				...
				foreach ( $results as $row ) {
					...
					$returns['kpi']['top_prod']['qty'][] = array( 'id' => $id,
																	'title' => '-',
																	'sales' => $row['sales'], 
																	'qty' => $row['qty'] );
					$keys[] = $id;
				...
				$prod_cond = (!empty($t_pq_ids)) ? ' AND ( (product_id IN ('. implode(",",$t_pq_ids) .') AND variation_id = 0)' : '';
				$prod_cond .= (!empty($v_ids)) ? ( (!empty($prod_cond)) ? ' OR ' : ' AND ( ' ) . 'variation_id IN ('. implode(",",$v_ids) .')' : '';
				$prod_cond .= (!empty($prod_cond)) ? ' ) ' : '';
			...
			$data = sr_get_prod_title(array( &$returns['kpi']['top_prod']['sales'], &$returns['kpi']['top_prod']['qty'], &$returns['kpi']['top_aprod'] ), 
		...
		if( !empty($post['cmd']) && $post['cmd'] == 'aprod_export' ) {
			...
			if ( count(array_keys($r_aprod['a_prod'])) > 0 ) {
				...
				sr_get_prod_title(array( &$r_aprod['a_prod'] ), 
								array('t_p_ids' => $t_p_ids, 't_v_ids' => $t_v_ids, 'security' => $params['security']) );
