...
class PW_COST_GOOD_framework {


	/** @var PW_COST_GOOD_framework The single instance of the class */
	 ...
	 *
	 * @since 2.0.0
	 * @return PW_COST_GOOD_framework singleton instance
	 */
	...
	public function load_framework_plugins() {
		...
		foreach ( $this->registered_plugins as $plugin ) {
			...
			if ( ! class_exists( 'PW_COST_GOOD_ASSIST' ) ) {
				require_once( $this->get_plugin_path( $plugin['path'] ) . '/lib/Proword/woocommerce/PW-assist.php' );
			...
			if ( isset( $plugin['args']['is_payment_gateway'] ) && ! class_exists( 'SV_WC_Payment_Gateway' ) ) {
				require_once( $this->get_plugin_path( $plugin['path'] ) . '/lib/Proword/woocommerce/payment-gateway/class-sv-wc-payment-gateway-plugin.php' );
			}
		 ...
		 * @since 2.0.0
		 */
		do_action( 'PW_COST_GOOD_ASSIST_LOAD' );
	}
...
PW_COST_GOOD_framework::instance();
