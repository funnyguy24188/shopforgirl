...
if ( ! class_exists( 'PW_COST_GOOD_ADMIN_BASE_APLI' ) ) :


abstract class PW_COST_GOOD_ADMIN_BASE_APLI {
	 ...
	 *
	 * @since 2.2.0
	 * @param object $request class instance which implements \PW_COST_GOOD_ADMIN_API_REQUEST
	 * @throws Exception
	 * @throws \PW_COST_GOOD_ASSIST_API
	 * @return object class instance which implements \PW_COST_GOOD_ADMIN_RESPONSE_APLI
	 */
	protected function perform_request( $request ) {
		...
		} catch ( PW_COST_GOOD_ASSIST_EXCEPTION $e ) {
	 ...
	 * @throws \PW_COST_GOOD_ASSIST_API network issues, timeouts, API errors, etc
	 * @return object request class instance that implements PW_COST_GOOD_ADMIN_API_REQUEST
	 */
	protected function handle_response( $response ) {

		// check for WP HTTP API specific errors (network timeout, etc)
		if ( is_wp_error( $response ) ) {
			throw new PW_COST_GOOD_ASSIST_API( $response->get_error_message(), (int) $response->get_error_code() );
		}
	 ...
	 * processing should continue, or throw a \PW_COST_GOOD_ASSIST_API with a
	 ...
	 * @return object response class instance which implements PW_COST_GOOD_ADMIN_API_REQUEST
	 */
	...
	protected function broadcast_request() {
		 ...
		 * }
		 * @param \PW_COST_GOOD_ADMIN_BASE_APLI $this instance
		 */
	...
	protected function get_request_uri() {
		 ...
		 * @param \PW_COST_GOOD_ADMIN_BASE_APLI class instance
		 */
	 ...
	 *
	 * @since 2.2.0
	 * @see \PW_COST_GOOD_ADMIN_API_REQUEST
	 ...
	 *
	 * @since 2.2.0
	 * @see \PW_COST_GOOD_ADMIN_RESPONSE_APLI
	 ...
	 * \PW_COST_GOOD_ADMIN_API_REQUEST which should be used in the child class API methods
	 * to build the request. The returned PW_COST_GOOD_ADMIN_API_REQUEST should be passed
	 * to self::perform_request() by your concrete API methods
	 *
	 * @since 2.2.0
	 * @param array $args optional request arguments
	 * @return \PW_COST_GOOD_ADMIN_API_REQUEST
	 */
	...
	public function set_tls_1_2_request( $handle, $r, $url ) {
		...
		if ( ! PW_COST_GOOD_ADMIN_HELPER::str_starts_with( $url, 'https://' ) ) {
			return;
		}
