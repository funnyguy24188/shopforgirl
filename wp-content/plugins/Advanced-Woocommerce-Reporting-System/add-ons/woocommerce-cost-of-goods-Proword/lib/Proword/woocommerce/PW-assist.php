...
if ( ! class_exists( 'PW_COST_GOOD_ASSIST' ) ) :


abstract class PW_COST_GOOD_ASSIST {
	...
	/** @var  \PW_COST_GOOD_ADMIN_HANDLER_MGS instance */
	...
	/** @var PW_COST_GOOD_ADMIN_HANDLER_NOTICE the admin notice handler class */
	...
	public function __construct( $id, $version, $args = array() ) {
		...
		add_action( 'PW_COST_GOOD_ASSIST_LOAD', array( $this, 'lib_includes' ) );
		...
		if ( is_admin() && ! is_ajax() ) {
			...
			require_once( $this->get_framework_path() . '/PW-admin-message-handler.php' );
		...
		// automatically log HTTP requests from PW_COST_GOOD_ADMIN_BASE_APLI
	...
	private function includes() {
		...
		require_once(  $framework_path . '/PW-plugin-exception.php' );

		// common utility methods
		require_once( $framework_path . '/PW-helper.php' );

		// backwards compatibility for older WC versions
		require_once( $framework_path . '/PW-plugin-compatibility.php' );

		if ( is_admin() ) {
			// load admin notice handler
			require_once( $framework_path . '/PW-admin-notice-handler.php' );
		}

		// generic API base
		require_once( $framework_path . '/api/PW-api-exception.php' );
		require_once( $framework_path . '/api/PW-api-base.php' );
		require_once( $framework_path . '/api/PW-api-request.php' );
		require_once( $framework_path . '/api/PW-api-response.php' );

		// XML API base
		require_once( $framework_path . '/api/PW-api-xml-request.php' );
		require_once( $framework_path . '/api/PW-api-xml-response.php' );

		// JSON API base
		require_once( $framework_path . '/api/PW-api-json-request.php' );
		require_once( $framework_path . '/api/PW-api-json-response.php' );
	}
	...
	private function load_hook_deprecator() {
		...
		require_once( $this->get_framework_path() . '/PW-hook-deprecator.php' );

		$this->hook_deprecator = new PW_COST_GOOD_ADMIN_DEPRECATOR( $this->get_plugin_name(), $this->get_deprecated_hooks() );
	}
	...
	/**
	 * Automatically log API requests/responses when using PW_COST_GOOD_ADMIN_BASE_APLI
	 *
	 * @since 2.2.0
	 * @see PW_COST_GOOD_ADMIN_BASE_APLI::broadcast_request()
	 */
	 ...
	 *
	 * @since 2.2.0
	 * @param array $request request data, see PW_COST_GOOD_ADMIN_BASE_APLI::broadcast_request() for format
	...
	public function get_admin_notice_handler() {
		...
		return $this->admin_notice_handler = new PW_COST_GOOD_ADMIN_HANDLER_NOTICE( $this );
	}
	 ...
	 *
	 * @since 2.0.0
	 * @see PW_COST_GOOD_ASSIST::get_settings_url()
	 ...
	 *
	 * @since 2.0.0
	 * @see PW_COST_GOOD_ASSIST::get_settings_link()
	 * @param string $plugin_id optional plugin identifier.  Note that this can be a
	 *        sub-identifier for plugins with multiple parallel settings pages
	 *        (ie a gateway that supports both credit cards and echecks)
	 ...
	 *
	 * @since 2.0.0
	 * @return PW_COST_GOOD_ADMIN_HANDLER_MGS
	 */
	public function get_message_handler() {
		...
		return $this->message_handler = new PW_COST_GOOD_ADMIN_HANDLER_MGS( $this->get_id() );
	}
	...
	public function is_plugin_active( $plugin_name ) {
		...
		foreach ( $active_plugins as $plugin ) {
			...
			if ( PW_COST_GOOD_ADMIN_HELPER::str_exists( $plugin, '/' ) ) {
