 ...
 * @since 1.0
 */
class PW_COST_GOOD_ADMIN {


	/** @var \PW_COST_GOOD_ADMIN_ORDERS class instance */
	protected $orders;

	/** @var \PW_COST_GOOD_ADMIN_PRODUCTS class instance */
	protected $products;
	...
	protected function load_classes() {
		...
		$this->orders   = PW_COST_GOODS()->load_class( '/includes/admin/PW-cog-admin-orders.php', 'PW_COST_GOOD_ADMIN_ORDERS' );
		$this->products = PW_COST_GOODS()->load_class('/includes/admin/PW-cog-admin-products.php', 'PW_COST_GOOD_ADMIN_PRODUCTS' );
	}
	 ...
	 *
	 * @since 2.0.0
	 * @return \PW_COST_GOOD_ADMIN_ORDERS
	 */
	 ...
	 *
	 * @since 2.0.0
	 * @return \PW_COST_GOOD_ADMIN_PRODUCTS
	 */
	...
	public static function get_global_settings() {
		...
		return apply_filters( 'wc_cog_global_settings', array(
			...
			// section start
			array(
				'name' => __( 'Cost of Goods Options', PW_COST_GOOD_TEXTDOMAIN ),
				'type' => 'title',
			...
			// include fees
			array(
				'title'         => __( 'Exclude these item(s) from income when calculating profit. ', PW_COST_GOOD_TEXTDOMAIN ),
				'desc'          => __( 'Fees charged to customer (e.g. Checkout Add-Ons, Payment Gateway Based Fees)', PW_COST_GOOD_TEXTDOMAIN ),
			...
			array(
				'desc'          => __( 'Shipping charged to customer', PW_COST_GOOD_TEXTDOMAIN ),
			...
			// include taxes
			array(
				'desc'          => __( 'Tax charged to customer', PW_COST_GOOD_TEXTDOMAIN ),
	...
	public function render_apply_costs_section( $field ) {
		...
		<tr valign="top">
			<th scope="row" class="titledesc">
				<label for="<?php echo esc_attr( $field['id'] ); ?>"><?php esc_html_e( 'Apply Costs to Previous Orders', PW_COST_GOOD_TEXTDOMAIN ); ?></label>
				<?php echo PW_COST_GOOD_ASSIST_COMPATIBLE::wc_help_tip( __( 'This will apply costs to previous orders based on your selection when "Apply Costs" is clicked and cannot be reversed.', PW_COST_GOOD_TEXTDOMAIN ) ); ?>
			</th>
			<td class="forminp forminp-<?php echo sanitize_html_class( $field['type'] ) ?>">
				<fieldset>
					<ul>
						<li><label><input name="wc_cog_apply_costs_option" id="wc_cog_apply_cost_option" type="radio" checked="checked" /><?php esc_html_e( 'Apply costs to orders that do not have costs set', PW_COST_GOOD_TEXTDOMAIN );?></label></li>
						<li><label><input name="wc_cog_apply_costs_option" id="wc_cog_apply_costs_option_all" type="radio" /><?php esc_html_e( 'Apply costs to all orders, overriding previous costs.', PW_COST_GOOD_TEXTDOMAIN );?></label></li>
					</ul>
					<span class="description"><?php esc_html_e( 'Choose carefully, there is no reversing either action!', PW_COST_GOOD_TEXTDOMAIN ); ?></span>
				</fieldset>
				<fieldset>
					<a href="<?php echo esc_url( add_query_arg( array( 'action' => 'apply_costs' ) ) ); ?>" class="button" id="<?php echo esc_attr( $field['id'] ); ?>"><?php esc_html_e( 'Apply Costs', PW_COST_GOOD_TEXTDOMAIN ); ?></a>
				</fieldset>
			</td>
		</tr>
		<?php
	...
	public function render_apply_costs_javascript() {
		...
		if ( PW_COST_GOODS()->is_plugin_settings() ) {

			$confirm_message = __( 'Are you sure you want to apply costs to all previous orders that have not already had costs generated? This cannot be reversed! Note that this can take some time in shops with a large number of orders, if an error occurs, simply Apply Costs again to continue the process.', PW_COST_GOOD_TEXTDOMAIN );
			$confirm_message_all = __( 'Are you sure you want to apply costs to all previous orders, overriding those with existing costs? This cannot be reversed! Note that this can take some time in shops with a large number of orders, if an error occurs, simply Apply Costs again to continue the process.', PW_COST_GOOD_TEXTDOMAIN );
	...
	public function handle_settings_actions() {
		...
		if ( PW_COST_GOODS()->is_plugin_settings() ) {
			...
			if ( 'apply_costs' === $current_action || 'apply_costs_all' === $current_action ) {
				...
				$success_count  = 0;
				$offset         = get_option( 'PW_COST_GOOD_OFFSET', 0 );
				$posts_per_page = 500;
				...
				do {
					...
					if ( is_wp_error( $order_ids ) ) {
						...
						PW_COST_GOODS()->get_message_handler()->add_error( __( 'Database error while applying product costs.', PW_COST_GOOD_TEXTDOMAIN ) );
					...
					if ( is_array( $order_ids ) ) {
						...
						foreach ( $order_ids as $order_id ) {
							...
							// set costs
							PW_COST_GOODS()->set_order_cost_meta( $order_id );
					...
					if ( 'apply_costs_all' === $current_action ) {
						...
						update_option( 'PW_COST_GOOD_OFFSET', $offset );
					}
				...
				delete_option( 'PW_COST_GOOD_OFFSET' );

				// success message
				/* translators: Placeholders: %d - number of orders updated. */
				PW_COST_GOODS()->get_message_handler()->add_message( sprintf( _n( '%d order updated.', '%d orders updated.', $success_count, PW_COST_GOOD_TEXTDOMAIN ), $success_count ) );

				// clear report transients
				PW_COST_GOODS()->get_admin_reports_instance()->clear_report_transients();
	...
	public function render_settings_actions_messages() {
		...
		if ( PW_COST_GOODS()->is_plugin_settings() ) {
			...
			PW_COST_GOODS()->get_message_handler()->show_messages();
		}
	}
	...
	public function load_message_handler() {
		...
		PW_COST_GOODS()->get_message_handler();
	}
	...
	public function load_styles_scripts( $hook_suffix ) {
		...
		if ( in_array( $post_type, array( 'product', 'shop_order' ) ) && in_array( $hook_suffix, array( 'edit.php', 'post.php', 'post-new.php' ) ) ) {
			...
			wp_enqueue_script( 'wc-cog-admin', PW_COST_GOODS()->get_plugin_url() . '/assets/js/admin/custom-js.js', $dependencies, PW_COST_GOOD_MAINCLASS::VERSION );
