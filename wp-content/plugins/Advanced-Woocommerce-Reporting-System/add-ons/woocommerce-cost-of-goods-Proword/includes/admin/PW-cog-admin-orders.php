 ...
 * @since 2.0.0
 */
class PW_COST_GOOD_ADMIN_ORDERS {
	...
	protected function init_hooks() {
		...
		// hide the _PW_COST_GOOD_ITEM_COST and _PW_COST_GOOD_ITEM_TOTAL_COST item meta on the Order Items table
	...
	public function add_order_item_cost_column_headers( $order ) {
		...
		<th class="item_cost_of_goods sortable" data-sort="float">
			<?php esc_html_e( 'Cost of Goods', PW_COST_GOOD_TEXTDOMAIN ); ?>
		</th>
		<?php
	}
	...
	public function add_order_item_cost( $product, $item, $item_id ) {
		...
		$item_cost = wc_get_order_item_meta( $item_id, '_PW_COST_GOOD_ITEM_TOTAL_COST', true );

		// set default cost if item cost doesn't exist
		if ( false === $item_cost ) {
			$item_cost = floatval( PW_COST_GOOD_PRODUCT::get_cost( $product ) ) * $item_qty;
		}
	...
	protected function get_total_cost_refunded_for_item( $item_id ) {
		...
		$total = $wpdb->get_var( $wpdb->prepare( "
			...
			WHERE order_itemmeta.meta_key = '_PW_COST_GOOD_ITEM_TOTAL_COST' AND
		...
		if ( PW_COST_GOOD_ASSIST_COMPATIBLE::is_wc_version_gte_2_6() ) {
	...
	public function update_order_cost_totals( $order_id, $order_item_costs ) {
		...
		foreach ( $order_item_costs as $order_item_id => $item_total_cost ) {
			...
			wc_update_order_item_meta( $order_item_id, '_PW_COST_GOOD_ITEM_TOTAL_COST', wc_format_decimal( $item_total_cost, 4 ) );
	...
	public function ajax_add_order_line_cost( $item_id, $item ) {
		...
		wc_add_order_item_meta( $item_id, '_PW_COST_GOOD_ITEM_COST', wc_format_decimal( $item_cost, 4 ) );
		...
		wc_add_order_item_meta( $item_id, '_PW_COST_GOOD_ITEM_TOTAL_COST', wc_format_decimal( $item_cost, 4 ) );
	}
	...
	public function hide_order_item_cost_meta( $hidden_fields ) {
		return array_merge( $hidden_fields, array( '_PW_COST_GOOD_ITEM_COST', '_PW_COST_GOOD_ITEM_TOTAL_COST' ) );
	}
	...
	public function show_order_total_cost( $post_id ) {
		?>
			<tr>
				<td class="total cost-total"><?php esc_html_e( 'Cost of Goods', PW_COST_GOOD_TEXTDOMAIN ); ?>:</td>
				<?php if ( PW_COST_GOOD_ASSIST_COMPATIBLE::is_wc_version_gte_2_6() ) : ?>
				<td></td>
				<?php endif; ?>
				<td class="total cost-total"><?php echo $this->get_formatted_order_total_cost( $post_id ); ?></td>
				<?php if ( PW_COST_GOOD_ASSIST_COMPATIBLE::is_wc_version_lt_2_6() ) : ?>
				<td width="1%"></td>
				<?php endif; ?>
			</tr>
		<?php
	...
	public function add_refund_order_costs( $refund_id ) {
		...
		foreach ( $refund->get_items() as $refund_line_item_id => $refund_line_item ) {
			...
			$item_cost = wc_get_order_item_meta( $refund_line_item['refunded_item_id'], '_PW_COST_GOOD_ITEM_COST', true );
			...
			wc_update_order_item_meta( $refund_line_item_id, '_PW_COST_GOOD_ITEM_COST',       wc_format_decimal( $refunded_item_cost ) );
			wc_update_order_item_meta( $refund_line_item_id, '_PW_COST_GOOD_ITEM_TOTAL_COST', wc_format_decimal( $refunded_item_total_cost ) );
