 ...
 * Single post
 */
//add_action( 'woocommerce_single_product_summary', 'theshopier_single_post_meta_bottom_sharing', 39 );
//add_action( 'theshopier_single_post_meta_bottom', 'theshopier_single_post_meta_bottom_sharing', 10 );
...
add_action( 'woocommerce_after_shop_loop_item', 'theshopier_shop_loop_hover_div_end', 100000 );
...
if( class_exists( 'YITH_Woocompare_Frontend' ) && class_exists( 'YITH_Woocompare' ) ) {
    ...
    function theshopier_add_compare_link(){
        ...
        printf( '<a href="%s" class="%s" data-product_id="%d">%s</a>', $fontend->add_product_url( $product_id ), 'nth-compare' . ( $is_button == 'button' ? ' button' : '' ), $product_id, $button_text );
    }
}
    ...
    function theshopier_product_labels(){
        ...
        if($product->is_on_sale()) {
            ...
            if( $product->regular_price > 0 ) {
                $_off_percent = (round($product->get_price() / $product->regular_price, 2) - 1)*100;
...
function theshopier_woocommerce_template_single_stock(){
    global $product;
    if( strcmp( $product->product_type, 'simple' ) == 0 ) {
...
if( !function_exists( 'theshopier_woocommerce_cart_item_remove_link_callback' ) ) {
    ...
    function theshopier_woocommerce_cart_item_remove_link_callback( $res, $cart_item_key ){
        ...
        return sprintf( '<a href="%s" class="remove nth_remove_cart" title="%s" data-key="%s" data-nonce="%s">'. esc_html__('Remove', 'theshopier').'</a>', esc_url( WC()->cart->get_remove_url( $cart_item_key ) ), esc_attr__( 'Remove this item', 'theshopier' ), $cart_item_key, $nonce );
    }
}
...
add_filter('woocommerce_product_gallery_attachment_ids', 'theshopier_woocommerce_product_gallery_attachment_ids', 10, 2);

function theshopier_woocommerce_product_gallery_attachment_ids($attachment_ids, $product){
    array_unshift($attachment_ids, get_post_thumbnail_id($product->ID));
    if($product->is_type('variable')) {
        $_variations = $product->get_available_variations();
        foreach($_variations as $data){
            array_push($attachment_ids, get_post_thumbnail_id($data['variation_id']));
        }
    }
