    ...
    <?php if ( count($products) > 0 ) :?>
        ...
        <?php foreach($products as $item):
            ...
            if( $product !== false && $product->exists() ) :
                ...
                $remove_link = $fontend->remove_product_url( $product->id );
            ?>

    <li data-row-id="<?php echo absint($product->id)?>">

        <a class="remove nth_remove_compare" href="<?php echo esc_url($remove_link);?>" data-product_id="<?php echo absint($product->id); ?>"><?php esc_html_e( 'Remove', 'theshopier' ) ?> <span class="remove">x</span></a>

        <a class="product-image" href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', absint($product->id) ) ) ) ?>">
            <?php echo $product->get_image() ?>
        </a>

        <div class="product-detail">
            <a class="product-title" href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', absint($product->id) ) ) ) ?>" title="<?php echo esc_attr( $product->get_title() ); ?>"><?php echo esc_html($product->get_title()); ?></a>
