...
//		return;
		...
		var totalbatches = 0;
		//create batch
		totalbatches = Math.ceil(selectedRows.length/50);
		if(!pbatch)
		{
			for(var ir=0; ir < selectedRows.length; ir++)
			{
...
//			$("#addproddialog").append('<div>'+trans_linkednote+'.</div>');
			$("#addproddialog").append('</br><div id="attributeplaceholder"><div id="divbuttonnext"><input id="buttonnext" class="button" type="button" value="Next >" /></div><div class="variationholder"> \
...
//		$("#addok").button("disable");
	...
	var _idmap = [
		...
		{id:'_product_permalink',field:'_product_permalink',name:'Product URL(permalink)',scope:SCOPE.NONE,width: 170,url:true,type:"text"},
		...
		{id:'_custom_attributes',field:'_custom_attributes',name:'Custom Attributes',width: 90,custattribute:true},
...
//				}
			...
			{
				curitem.name = params[field + 'name'];
				curitem.atslug = W3Ex._w3ex_map_attributes[curitem.name];
				curitem.slug = "";
				curitem.value = "";
					...
					for(var irow=0; irow < selectedRows.length; irow++)
					{
						...
						if(item.scope !== undefined && !_disablesafety)
						{
							...
							if(item.scope == SCOPE.PRODSVAR)
							{
								...
								{
									if(sellitem.haschildren !== undefined || sellitem.product_type === "variable")
									{
										continue;
									}
								}
				  ...
				  if(key === "_custom_attributes")
				  {
				  	   ...
				  	   if(selitem.post_type === 'product_variation')
					   {
					   		for (var i=0; i < selitem._custom_attributes.length; i++) 
							{
								 ...
								 if(valtoinsert === "")
								 {
								 	 valtoinsert+= attr.atslug + '#####' + attr.value;
								 }else
								 {
								 	 valtoinsert+= "*****" + attr.atslug + '#####' + attr.value;
								 }
